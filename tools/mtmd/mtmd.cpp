            ...
            case PROJECTOR_TYPE_GRANITE4V:
                {
                    ...
                    // mtmd_encode (PROJECTOR_TYPE_GRANITE4V branch).
                    img_beg = "<image>";
                    img_end = "";
                    image_preproc = std::make_unique<mtmd_image_preprocessor_llava_uhd>(ctx_v);
                } break;
    ...
    int32_t add_media(const mtmd_bitmap * bitmap) {
        if (!bitmap->is_audio) {
            ...
            if (
                ...
                if (ctx->proj_type_v() == PROJECTOR_TYPE_GRANITE4V) {
...
int32_t mtmd_encode(mtmd_context * ctx, const mtmd_image_tokens * image_tokens) {
    ...
    if (proj_type == PROJECTOR_TYPE_GRANITE4V) {
        ...
        const int n_slices       = clip_g4v_projector_count(ctx_clip);
        GGML_ASSERT(n_slices > 0 && n_mmproj_embd % n_slices == 0);
        const int projection_dim = n_mmproj_embd / n_slices;
        const float base_scale   = clip_g4v_base_stream_scale(ctx_clip);
